...
class GroupedQueryAttention(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, kv_n_heads: int, attn_impl: str='flash', clip_qkv: Optional[float]=None, qk_ln: bool=False, qk_gn: bool=False, softmax_scale: Optional[float]=None, attn_pdrop: float=0.0, norm_type: str='low_precision_layernorm', fc_type: Optional[dict[str, Any]]=None, device: Optional[str]=None, bias: bool=True, sliding_window_size: int=-1):
        ...
        if self.qk_ln or self.qk_gn:
            ...
            self.q_ln = build_norm(name=norm_type.lower(), normalized_shape=norm_size, device=device)
            if qk_ln:
                norm_size = self.head_dim * kv_n_heads
            self.k_ln = build_norm(name=norm_type.lower(), normalized_shape=norm_size, device=device)
    ...
    def get_implementation_specific_args(self, attention_mask: Optional[torch.Tensor]=None, alibi_slopes: Optional[torch.Tensor]=None, flash_attn_padding_info: Optional[dict[str, torch.Tensor]]=None) -> dict[str, Any]:
        ...
        if self.attn_impl == 'flash':
            extra_attn_kwargs = {'should_repeat_kv_for_gqa': not is_flash_v2_installed(), 'sliding_window_size': self.sliding_window_size, 'alibi_slopes': alibi_slopes, 'flash_attn_padding_info': flash_attn_padding_info, 'key_padding_mask': None}
        else:
