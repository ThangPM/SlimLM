...
def _megablocks_sparse_mlp_generic_param_init_fn_(module: nn.Module, init_fn_: Callable, init_div_is_residual: bool=False, div_is_residual: float=1.0):
    ...
    if module.weight_parallel_group is not None:
        weight_parallel_group_size = int(module.weight_parallel_group.size())
        weight_parallel_group_rank = int(module.weight_parallel_group.rank())
    hidden_size = int(module.hidden_size)
    w1 = module.w1
    if isinstance(w1, DTensor):
        w1 = w1._local_tensor
    w1_size = list(w1.shape)
    w1_size[0] = w1_size[0] * expert_process_group_size * weight_parallel_group_size
    ...
    _w1_local_slice = _w1_local.chunk(weight_parallel_group_size, dim=0)[weight_parallel_group_rank]
    with torch.no_grad():
        w1.copy_(_w1_local_slice)
    w2 = module.w2
    if isinstance(w2, DTensor):
        w2 = w2._local_tensor
    w2_size = list(w2.shape)
    w2_size[0] = w2_size[0] * expert_process_group_size * weight_parallel_group_size
    ...
    _w2_local_slice = _w2_local.chunk(weight_parallel_group_size, dim=0)[weight_parallel_group_rank]
    with torch.no_grad():
        w2.copy_(_w2_local_slice)
...
def _megablocks_sparse_glu_generic_param_init_fn_(module: nn.Module, init_fn_: Callable, init_div_is_residual: bool=False, div_is_residual: float=1.0):
    ...
    v1_size[0] = v1_size[0] * expert_process_group_size * weight_parallel_group_size
    ...
    _v1_local_slice = _v1_local.chunk(weight_parallel_group_size, dim=0)[weight_parallel_group_rank]
    with torch.no_grad():
        v1.copy_(_v1_local_slice)
...
def _megablocks_mlp_generic_param_init_fn_(module: nn.Module, init_fn_: Callable, init_div_is_residual: bool=False, div_is_residual: float=1.0):
    ...
    if module.weight_parallel_group is not None:
        weight_parallel_group_size = int(module.weight_parallel_group.size())
        w_rank = int(module.weight_parallel_group.rank())
    ...
    with torch.no_grad():
        module.w1.copy_(_w1_local_slice)
    ...
    with torch.no_grad():
        module.w2.copy_(_w2_local_slice)
    if init_div_is_residual is not False:
        with torch.no_grad():
